...
def find_start_end(arr, start_char='S', end_char='E'):
    start_row, start_col = np.nonzero(arr == start_char)[0][0], np.nonzero(arr == start_char)[1][0]
    end_row, end_col = np.nonzero(arr == end_char)[0][0], np.nonzero(arr == end_char)[1][0]
...
def part_one(my_list):
    ...
    shortest_path = G.subgraph(nx.astar_path(G, f"{start_row,start_col}", f"{end_row,end_col}"))
...
if __name__ == "__main__":
    with open("C:/Users/joshu/OneDrive/Desktop/Advent of Code 2022/Day12/input.txt", "r") as f:
        ...
